...
class BucketInsert extends Component {
  ...
  handleOnKeyPress = ev => {
    ...
    if (ev.key == "Enter") {
      ...
      // bucket_add(this.state.bucket_title); 정형적 (분해하지 않았을때)
      bucket_add(bucket_title); // 비정형적, 비구조적(분해후 독립변수 사용)
      this.setState({
        bucket_title: ""
      });
    }
    ...
    return (
      <section className="w3-container-fluid">
        <div className="w3-form-control w3-margin">
           ...
           * => input box는 read only상태로 변한다.
           * 3. 사용자의 d입력을 받아서 state 변수에 저장할 수 있도록
