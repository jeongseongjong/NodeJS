...
router.post("/upload", function (req, res) {
  saveFile(req, res, function (error) {
    if (error) res.send("파일업로드 오류");

    // multer가 파일을 정상적으로 upload 하고나면
    // req.file 변수에 값들을 설정(Setting)해 놓는다.
    // req.fjile 변수에 있는 속성값들을 가지고 DB저장을 수행
    // if(req.file) : req.file 값이 설정되어 있으면

    if (req.file) {
      let originalname = req.file.originalname;

      // multer 의 filename에 의해서 변경된 파일이름
      // 실제 업로드 저장된 파일이 된다.
      let photoname = req.file.filename;

      /**
       * form에서 업로드한pText, pTitle은 req.body에 담겨서 여기까지
