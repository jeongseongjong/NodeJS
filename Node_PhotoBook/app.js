...
/**
  require() 확장모듈, 미들웨어 등을 impot하는 명령함수
  ESS 이하에서 js파일을 외부에 분리하고
  실제 사용할 때는 import 사용하는 방식

  ES6 이상에서는 import 키워드로 사용이 되는데
  아직 node12버전 이하에서는 import 키워드를 지원하지 않는다.

  node 14버전이상에서는 import 키워드를 공식으로 지원

  require를 사용하여 import를 할 때
...
// spring에서 Controller역할을 수행하는 모듈
app.use("/", indexRouter);
